const modelOptions = {
  ...
  Honda: ["Civic","Jazz","CR-V","CBR500"], // μηχανές Honda
  Yamaha: ["MT-07","R1","XSR900"]          // μηχανές Yamaha
};
...
function loadVehicles() {
  ...
  Object.keys(vehicles).forEach(key => {
    ...
    `;
    div.onclick = (e) => {
      if(e.target.classList.contains("delete-btn")) return; // Αν είναι το Χ, μην ανοίγει
      loadVehicle(key);
    }
  ...
  document.querySelectorAll(".delete-btn").forEach(btn => {
    btn.addEventListener("click", (e) => {
      const parent = e.target.closest(".vehicle-item");
      const key = parent.dataset.key;
      let vehicles = JSON.parse(localStorage.getItem("vehicles") || "{}");
      delete vehicles[key];
      localStorage.setItem("vehicles", JSON.stringify(vehicles));
      loadVehicles();
      e.stopPropagation(); // για να μην ενεργοποιεί το onclick του div
    });
  });

  // Επισήμανση πρώτου
  const first = vehicleList.querySelector(".vehicle-item");
  if(first) loadVehicle(first.dataset.key);
}
